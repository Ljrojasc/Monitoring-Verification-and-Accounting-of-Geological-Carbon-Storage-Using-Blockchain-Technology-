...
class CcsChaincode extends Contract {
  ...
  async CreateCaptureContract(ctx, ccsData) {
    ...
    //To simulate transactions for different months without modifying chaincode logic elsewhere.
    //Purpose: Makes createdAt optional in the payload. If user provides a date, it uses that.
    //If user does not provide a date, it defaults to the current timestamp.

    data.createdAtTest = data.createdAt ? new Date(data.createdAt).toISOString() : new Date().toISOString(); 
  ...
  async UpdateTransportDetails(ctx, transportData) {
    ...
    data.docType = CONTRACT_DOC_TYPE;
    data.createdBy = cid.getID();
    data.createdAt = data.createdAt ? new Date(data.createdAt).toISOString() : new Date().toISOString();
